...
from mineru.utils.usage_logger import usage_logger
...
class AsyncTaskManager:
    ...
    async def _process_task(self, task_id: str) -> None:
        ...
        try:
            ...
            # Log failure
            try:
                duration = (datetime.fromisoformat(task.completed_at) - datetime.fromisoformat(task.started_at)).total_seconds()
                usage_logger.log_usage(
                    task_id=task.task_id,
                    ip_address="N/A",
                    file_name=", ".join(task.file_names),
                    page_count=0,
                ...
                logger.error(f"Failed to log failure usage for task {task.task_id}: {e}")
    ...
    async def _run_task(self, task: AsyncParseTask) -> None:
        ...
        try:
            ...
            usage_logger.log_usage(
                task_id=task.task_id,
                ip_address="N/A", # IP address can be passed if available
                file_name=", ".join(task.file_names),
                page_count=0, # total pages can be calculated if needed
                duration_sec=duration,
                status=task.status
            )
        except Exception as e:
            logger.error(f"Failed to log usage for task {task.task_id}: {e}")
